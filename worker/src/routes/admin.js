...
async function overview(ctx) {
    ...
    const counts = await one(db, `
        SELECT
            ...
            (SELECT COALESCE(SUM(views), 0) FROM page_views) AS views,
    ...
    const [perSite, daily, dailyLikes, dailyViews, topPages, latest] = await Promise.all([
        ...
        all(db, `SELECT substr(created_at, 1, 10) AS day, COUNT(*) AS likes FROM likes
                 WHERE substr(created_at, 1, 10) >= ? GROUP BY day ORDER BY day`, since),
        all(db, `SELECT day, SUM(views) AS views FROM page_views WHERE day >= ? GROUP BY day ORDER BY day`, since),
    ...
    return json({ totals: counts, perSite, daily, dailyLikes, dailyViews, topPages, latest: latest.map(adminComment) });
}
...
async function updateSite(ctx, id, body) {
    ...
    await run(ctx.db, `UPDATE sites SET name = ?, origins = ?, moderation = ?, allow_anonymous = ?,
        ...
        next.commentsOn, next.likesOn, next.viewsOn, next.locale, next.active, id);
...
async function deleteSite(ctx, id) {
    ...
    await ctx.db.batch([
        ...
        ctx.db.prepare('DELETE FROM page_views WHERE site_id = ?').bind(id),
...
async function listViews(ctx, url) {
    const siteId = num(url.searchParams.get('site'), 0);
    const clause = siteId ? 'WHERE v.site_id = ?' : '';
    const todayKey = new Date().toISOString().slice(0, 10);
    const args = siteId ? [siteId] : [];

    const pages = await all(ctx.db, `
        ...
        ${clause}
        GROUP BY v.site_id, v.page_path ORDER BY views DESC LIMIT 200`, todayKey, ...args);

    const totals = await one(ctx.db, `
        SELECT COALESCE(SUM(views), 0) AS total,
               COALESCE(SUM(CASE WHEN day = ? THEN views END), 0) AS today
        FROM page_views v ${clause}`, todayKey, ...args);

    return json({ pages, totals });
}
...
export async function handleAdminRoute(ctx, request, url) {
    ...
    if (route === '/views' && method === 'GET') return listViews(ctx, url);
